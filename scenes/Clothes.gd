...
func _ready():
	...
	#need to color black
	change_color(Color(randf(), randf(), randf()))
...
func change_symbol(new_tile_index):
	...
	#Turn off old one...
	var temp_color = get_child(0).get_child(tile_index).modulate #save the color
	get_child(0).get_child(tile_index).visible = false
	
	#Change to new one
	tile_index = new_tile_index
	get_child(0).get_child(tile_index).visible = true
	get_child(0).get_child(tile_index).modulate = temp_color
	


#function to change the color of the proper tile
func change_color(color):
	get_child(0).get_child(tile_index).modulate = color